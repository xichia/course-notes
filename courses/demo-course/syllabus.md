---
id: demo-course-syllabus
title: Synthetic Demo Syllabus
course: demo-course
type: reference
topic: syllabus
aliases: [demo syllabus]
prerequisites: []
related: [demo-layered-recall]
exam-weight: none
status: reference
last-reviewed:
review-after:
source: "Synthetic framework example written for this repository"
visibility: public-framework
source-risk: original
---

# Synthetic Demo Syllabus

> This is fictional framework data, not an official syllabus or summary of a real course.

## Official Learning Outcomes

- Demo outcome: navigate from the generated manifest to one selected synthetic concept.
- Demo outcome: update review metadata after an evidence-based practice attempt.

## Weekly Topics

| Week | Synthetic topic | Source |
|---:|---|---|
| 1 | Repository navigation | Synthetic framework example |
| 2 | Review workflow | Synthetic framework example |

## Assessment Topics

_None—this fictional course has no assessment._

## Exam-Relevant Sections

_None—this fictional course has no exam._

## Excluded Topics

_All real university course material is excluded._

## Lecturer Hints

_None—there is no lecturer._

## Source Links or References

- Synthetic content authored for this public framework candidate.
